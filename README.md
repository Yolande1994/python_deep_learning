...
### 项目亮点  
- 底层原理扎实：经典分类器与损失函数全部基于 NumPy 从零实现，包含完整梯度推导，深入理解算法本质
- 工程化闭环：从数据划分、网络搭建、训练验证到模型保存，完整复现工业级项目开发流程
- 迁移学习落地：基于 ImageNet 预训练模型实现全参数微调，适配自定义二分类数据集
- 模块化目录设计：按职责拆分算法源码、工程项目、工具集、教程演示，结构清晰易维护
- 配套演示文档：Jupyter Notebook 分步验证算法效果，包含数据处理、参数调试与结果可视化
