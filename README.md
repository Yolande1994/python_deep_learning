...
## 项目简介
本仓库为深度学习算法学习与实战项目合集，覆盖从经典机器学习分类器底层原理实现，到卷积神经网络工程化训练、迁移学习落地的完整学习路径。所有算法均包含原理级手动实现与可运行的完整工程代码。
...
### 运行注意事项
1. 数据集文件夹 `datasets/`、`cat_dog_classify/datasets/`、`pretrain_weights/` 已配置 `.gitignore`，不会上传大体积文件，需自行下载对应数据集放置目录；
2. 代码默认自动检测GPU，无CUDA环境会自动切换CPU运行；
3. 预训练权重文件需自行下载放入 `cat_dog_classify/pretrain_weights/`。

### 仓库忽略规则说明
仓库根目录配置 `.gitignore`，自动屏蔽以下文件，不占用仓库存储空间：
- 数据集文件夹、预训练模型权重文件（`.pth`/`.pt`）
- Python缓存文件 `__pycache__`、`.pyc`
- Jupyter缓存 `.ipynb_checkpoints`
- IDE配置文件夹 `.vscode`、`.idea`
- 系统文件、虚拟环境目录
